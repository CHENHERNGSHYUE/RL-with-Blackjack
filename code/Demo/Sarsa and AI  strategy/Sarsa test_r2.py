...
from Blackjack_AI_strategy_r2 import Blackjack
from Sarsa_learning_r2 import Sarsa_table
import csv
import time
import matplotlib.pyplot as plt

def update(): # inspired by Morvan Z.
    for episode in range(10000): # set the testing rounds
        ...
        while True: # loop (playing times) within each round
            ...
            action_ = rl.choose_action(str(observation_)) # choose next action according to the next state instead of always select max Q
...
if __name__ == "__main__":
    ...
    #-------remove or not-------
    with open('strategy.csv', 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['Dealer shown card', 'Player scores', 'Current state', 'Action', 'Win or not'])
    ...
    rl.s_table.to_csv(r'export_Sarsatable.csv', header=True) # output Sarsa table
